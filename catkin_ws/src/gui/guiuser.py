...
from gui.msg import Sensordata
...
class GUI(QtGui.QMainWindow):
	...
	def rotRight(self):
		...
		self.rotRightBtn.setStyleSheet(
			'QPushButton {padding: 7px;background-color: #00FF00;color: #000000;}')
		self.subpub.publish("rotright")	
	def rotLeft(self):
		self.kommando.append(":Kappa:")
		self.rotLeftBtn.setStyleSheet(
			'QPushButton {padding: 7px;background-color: #00FF00;color: #000000;}')
		self.subpub.publish("rotleft")
	...
	def updateSensor(self, sensorData):
		...
		self.setGyro(sensorData.angle[0], sensorData.angle[1], sensorData.angle[2])
		self.irGrid.setTemp(sensorData.ir)
		self.setTOF(sensorData.dist)
